...
- **전역 실행 콘텍스트(Global Execution Context)**  
  전역 실행 콘텍스트는 기본 콘텍스트 혹은 base 실행 콘텍스트입니다. 어느 함수에도 포함되지 않은 코드 전역 실행 콘텍스트에 속하게 됩니다. 전역 실행 콘텍스트는 두 가지 작동을 하는데, (브라우저 환경인 경우) window 객체인 전역 객체를 생성하고 `this`의 값을 전역 객체와 동일하게 설정합니다. 자바스크립트 프로그램에는 하나의 전역 실행 콘텍스트만 존재합니다.
...
Javascript 엔진은 스택의 최상단에 있는 실행 콘텍스트의 함수를 실행합니다. 함수 실행이 완료되면 스택에서 제거되고 현재 스택의 아래에 있는 콘텍스트로 컨트롤이 도달합니다.
